...
var SessionDetailsView = Backbone.View.extend({
	...
	render : function(pid) {
		...
		// show tooltip
		$('#fullurl').tipsy({gravity: 'n', fade: true, html: true, delayOut: 500});
...
//	return r;
    try{
        localStorage.post = req.postData;
        if (!!req.postData){
            localStorage.postText = req.postData.text;
            return decodeAndHighlight(req.postData.text);
        }
        return decodeAndHighlight(req.queryString[0].name);
    } catch(e){
        return null;
    }
}
...
function getResponseParams(content) {
	return decodeAndHighlight(content);
}
function htmlEscape(str){
    return $('<div></div>').text(str).html();
}
function decodeAndHighlight(content) {
    try {
        ...
        // can't parse json
        // try base64 decod and parse again
        try {
            var base64decoded = Base64.decode(content);
            var json = $.parseJSON(base64decoded);
            var str = JSON.stringify(json, undefined, 2);
            return syntaxHighlight(str);
        } catch (e3){
            // try xml
            try {
                ...
                return "<span class='alert-error'>undefined</span><br/>" + htmlEscape(content);
            }
        }
    }
}
